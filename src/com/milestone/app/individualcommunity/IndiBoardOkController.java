...
public class IndiBoardOkController implements Execute{
	   public Result execute(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		      System.out.println("Indi boardOk컨트롤러");
		      ...
		      String temp = req.getParameter("page");                  //혹시라도 글이없을 경우 NPE가 뜨므로 현재상태를 담아주고 int page 연산후 저장
				...
				int total = individualCommunityDAO.selectCount();   //전체 게시글 수
//				한 페이지에 출력되는 게시글의 개수
				int rowCount = 4;
//				한 페이지에서 나오는 페이지 버튼의 개수
		      ...
		      req.setAttribute("boards", individualCommunityDAO.selectAll(pageMap));
		      ...
		      result.setPath("/app/board/indiBoard.jsp");
		      return result;
	   }
}
